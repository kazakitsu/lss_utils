...
def kaxes_1d(ng: int, boxsize: float, *, dtype=np.float32):
    """Return 1D physical k-axes for x,y (fftfreq) and z (rfftfreq)."""
    dtype = np.dtype(dtype)
    boxsize = np.asarray(boxsize, dtype)
    fac = (2.0 * np.pi) / boxsize
    # Use sample spacing d = 1/ng so that fac * fftfreq gives physical k.
    d = 1.0 / np.asarray(ng, dtype)
    kx = fac * np.fft.fftfreq(ng, d=d)
    ky = kx
    kz = fac * np.fft.rfftfreq(ng, d=d)
...
def _helicity_basis_real(kx, ky, kz, eps=1e-12):
    ...
    """
    rtype = np.result_type(kx, ky, kz)
    KX, KY, KZ = _k_broadcast(kx, ky, kz)
    # Compute k^2 and its norm with safe handling at k=0
    k2 = (KX * KX + KY * KY + KZ * KZ).astype(rtype)
    kn = np.sqrt(np.where(k2 > 0, k2, 1.0)).astype(rtype)

    # Unit vector khat
    KHX = np.where(k2 > 0, KX / kn, 0.0)
    KHY = np.where(k2 > 0, KY / kn, 0.0)
    KHZ = np.where(k2 > 0, KZ / kn, 0.0)

    # First try with n = z-hat: c1 = n × khat = (-KHY, KHX, 0)
    c1x, c1y = -KHY, KHX
    c1z = np.zeros_like(KHX)
    n1 = np.sqrt(c1x * c1x + c1y * c1y + c1z * c1z)
    use1 = (n1 > eps)

    # Fallback with n = x-hat: c2 = n × khat = (0, -KHZ, KHY)
    c2x = np.zeros_like(KHX)
    c2y = -KHZ
    c2z = KHY
    n2 = np.sqrt(c2x * c2x + c2y * c2y + c2z * c2z)

    invn1 = np.where(use1, 1.0 / n1, 0.0)
    invn2 = np.where(n2 > 0, 1.0 / n2, 0.0)

    e1x = np.where(use1, c1x * invn1, np.where(k2 > 0, c2x * invn2, 0.0))
    e1y = np.where(use1, c1y * invn1, np.where(k2 > 0, c2y * invn2, 0.0))
    e1z = np.where(use1, c1z * invn1, np.where(k2 > 0, c2z * invn2, 0.0))
...
def _project_speed_subset(
    ...
    # Cast basis to the same dtype as S6 to avoid implicit upcasts
    KHXc, KHYc, KHZc = KHX.astype(S6.dtype, copy=False), KHY.astype(S6.dtype, copy=False), KHZ.astype(S6.dtype, copy=False)
    e1x_c, e1y_c, e1z_c = e1x.astype(S6.dtype, copy=False), e1y.astype(S6.dtype, copy=False), e1z.astype(S6.dtype, copy=False)
    e2x_c, e2y_c, e2z_c = e2x.astype(S6.dtype, copy=False), e2y.astype(S6.dtype, copy=False), e2z.astype(S6.dtype, copy=False)

    ctype = np.result_type(S6.dtype, np.complex64)
    zero = np.zeros((), dtype=ctype)
    ...
    # m=0
    if need_m0:
        trS = _trace_S(S6).astype(S6.dtype, copy=False)
        s0 = (KHXc * Skx) + (KHYc * Sky) + (KHZc * Skz)
        s0 -= trS / 3.0
        s0 *= np.sqrt(1.5, dtype=s0.dtype)
        s0 = s0.astype(ctype, copy=False)
        outs[0] = np.where(mask, s0, zero)

    # m=±1
    if need_p1 or need_m1:
        a = (KHXc * Se1x) + (KHYc * Se1y) + (KHZc * Se1z)
        b = (KHXc * Se2x) + (KHYc * Se2y) + (KHZc * Se2z)
        a = a.astype(ctype, copy=False)
        b = b.astype(ctype, copy=False)
        if need_p1:
            outs[1] = np.where(mask, a - 1j * b, zero)
        if need_m1:
            outs[2] = np.where(mask, a + 1j * b, zero)

    # m=±2
    if need_p2 or need_m2:
        q11 = (e1x_c * Se1x) + (e1y_c * Se1y) + (e1z_c * Se1z)
        q22 = (e2x_c * Se2x) + (e2y_c * Se2y) + (e2z_c * Se2z)
        q12 = (e1x_c * Se2x) + (e1y_c * Se2y) + (e1z_c * Se2z)
        q11 = q11.astype(ctype, copy=False)
        q22 = q22.astype(ctype, copy=False)
        q12 = q12.astype(ctype, copy=False)
        d = 0.5 * (q11 - q22)
        if need_p2:
            outs[3] = np.where(mask, d - 1j * q12, zero)
        if need_m2:
            outs[4] = np.where(mask, d + 1j * q12, zero)
...
def _project_block_subset(
    ...
    KHXc, KHYc, KHZc = KHX.astype(S6_blk.dtype, copy=False), KHY.astype(S6_blk.dtype, copy=False), KHZ.astype(S6_blk.dtype, copy=False)
    e1x_c, e1y_c, e1z_c = e1x.astype(S6_blk.dtype, copy=False), e1y.astype(S6_blk.dtype, copy=False), e1z.astype(S6_blk.dtype, copy=False)
    e2x_c, e2y_c, e2z_c = e2x.astype(S6_blk.dtype, copy=False), e2y.astype(S6_blk.dtype, copy=False), e2z.astype(S6_blk.dtype, copy=False)

    ctype = np.result_type(S6_blk.dtype, np.complex64)
    zero = np.zeros((), dtype=ctype)
    ...
    if need_m0:
        trS = _trace_S(S6_blk).astype(S6_blk.dtype, copy=False)
        s0 = (KHXc * Skx) + (KHYc * Sky) + (KHZc * Skz)
        s0 -= trS / 3.0
        s0 *= np.sqrt(1.5, dtype=s0.dtype)
        s0 = s0.astype(ctype, copy=False)
...
def _loop_chunked_subset(
    ...
    """Python loop over z-chunks; each block is projected by _project_block_subset."""
    nx, ny, nz = S6.shape[1], S6.shape[2], S6.shape[3]
    # Determine output dtype once
    ctype = np.result_type(S6.dtype, np.complex64)
    ncomp = len(pack_order)
    out = np.zeros((ncomp, nx, ny, nz), dtype=ctype)
    ...
    while z0 < nz:
        ...
        # In-place assignment (NumPy) to avoid extra allocations
        out[..., z0:z1] = out_blk
        z0 = z1
    return out
